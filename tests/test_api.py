...
def test_terraclimate_temperature_map():
    ...
    with patch(
        "api.main._terraclimate_temperature_map", return_value=expected
    ) as temperature_map:
        response = client.get("/terraclimate/temperature/map?month=7")

    assert response.status_code == 200
    assert response.json() == expected
    temperature_map.assert_called_once_with(7)


@pytest.mark.parametrize("month", [0, 13])
def test_terraclimate_temperature_rejects_invalid_month(month):
    response = client.get(f"/terraclimate/temperature/map?month={month}")
...
def test_terraclimate_temperature_expression():
    ...
    with patch.dict("sys.modules", {"ee": ee}):
        ...
        date_collection = ee.ImageCollection.return_value.filterDate.return_value
        collection = date_collection.filter.return_value
        weighted_collection = collection.map.return_value
        monthly_temperature = weighted_collection.mean.return_value.rename.return_value
        masked_temperature = monthly_temperature.updateMask.return_value
        result = masked_temperature.clip.return_value
        result.reduceRegion.return_value.getInfo.return_value = {
            "mean_air_temperature_c_p2": 3.5,
            "mean_air_temperature_c_p98": 21.5,
        }
        result.getMapId.return_value = {
            "tile_fetcher": type(
                "TileFetcher",
        ...
        response = _terraclimate_temperature_map(7)
        ...
        ee.Filter.calendarRange.assert_called_once_with(7, 7, "month")
        date_collection.filter.assert_called_once_with(
            ee.Filter.calendarRange.return_value
        )
        mean_month = collection.map.call_args.args[0]
        month = MagicMock()
        mean_month_result = mean_month(month)
        ...
        assert mean_month_result is scaled_mean.return_value
        weighted_collection.mean.assert_called_once_with()
        monthly_temperature.updateMask.assert_called_once()
        countries = ee.FeatureCollection.return_value
        ee.FeatureCollection.assert_called_once_with("USDOS/LSIB_SIMPLE/2017")
        ee.Filter.eq.assert_called_once_with("country_na", "Chile")
        countries.filter.assert_called_once_with(ee.Filter.eq.return_value)
        ...
        result.reduceRegion.assert_called_once_with(
            reducer=ee.Reducer.percentile.return_value,
            geometry=countries.filter.return_value.geometry.return_value,
            scale=4638.3,
            bestEffort=True,
            maxPixels=10_000_000,
        )
        ee.Reducer.percentile.assert_called_once_with([2, 98])
        result.getMapId.assert_called_once_with(
            {
                "min": 3.5,
                "max": 21.5,
        ...
        assert response["stretch_percentiles"] == [2, 98]
        _terraclimate_temperature_map.cache_clear()
